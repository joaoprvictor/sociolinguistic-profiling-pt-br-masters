...
response = data$total_nonstandard_verb_conjugation
response
...
# in some variables, it was important to adjust the formula
adjusted_formula = " response~sex+age*schooling"
model_with_interaction_nb_ajusted = update(model_with_interaction_nb, adjusted_formula)
summary(model_with_interaction_nb_ajusted)

# anova testing - check if models are equal (choose the less complex)
anova(model_no_interaction_nb, model_with_interaction_nb_ajusted,model_with_interaction_nb, test = "Chisq")

# =======================================
# WITH DOUBLE INTERACTION

## adjusting the model formula
#model_nb = glm.nb(adjusted_formula, data = data)
#summary(model_nb)
#model_nb = glm(adjusted_formula, family = negative.binomial(0.6558))
#model_nb
#summary(model_nb)

#model_nb_adjusted = update(model_nb, adjusted_formula)

### variance
par(mfrow = c(2, 2))
plot(model_with_interaction_nb_ajusted) 
par(mfrow = c(1, 1))

### normality
simulationOutput <- simulateResiduals(fittedModel = model_with_interaction_nb_ajusted, plot = TRUE)


### Dispersion
testDispersion(model_with_interaction_nb_ajusted)
check_overdispersion(model_with_interaction_nb_ajusted)

#### dispersion parameter, ideal < 1.5
analisis = summary(model_with_interaction_nb_ajusted)
analisis$deviance/analisis$df.residual

### Zero inflation - same idea of dispersion
testZeroInflation(model_with_interaction_nb_ajusted)
check_zeroinflation(model_with_interaction_nb_ajusted)

### R2 - not so relevant for count data but we did it anyway
r2(model_with_interaction_nb_ajusted)

### collinearity
check_collinearity(model_with_interaction_nb_ajusted)

# observing the sex levels
m_means <- emmeans(model_with_interaction_nb_ajusted, ~ sex)
...
#age in eahc level of schooling
m_means <- emmeans(model_with_interaction_nb_ajusted, ~ age|schooling)
cld(m_means, Letters = letters)
m_means_response <- emmeans(model_with_interaction_nb_ajusted, ~ age|schooling, type="response")
cld(m_means_response, Letters = letters)
#BASIC PLOT WITH CONFIDENCE LIMITS
emmip(m_means, ~ age|schooling, CIs=T) 
#BASIC PLOT WITH CONFIDENCE LIMITS ON THE RESPONSE SCALE
emmip(m_means, ~ age|schooling, CIs=T, type="response")


#schooling in eahc level of age
m_means <- emmeans(model_with_interaction_nb_ajusted, ~ schooling|age)
cld(m_means, Letters = letters)
#BASIC PLOT WITH CONFIDENCE LIMITS
emmip(m_means, ~ schooling|age, CIs=T) 
#BASIC PLOT WITH CONFIDENCE LIMITS ON THE RESPONSE SCALE
emmip(m_means, ~ schooling|age, CIs=T, type="response")
