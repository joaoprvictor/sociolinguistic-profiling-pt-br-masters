...
response = data$total_nonstandard_verb_agreement
...
summary(model_nb)
model_nb = glm(adjusted_formula, family = negative.binomial(0.8689))
model_nb
summary(model_nb)
...
### normality
simulationOutput <- simulateResiduals(fittedModel = model_nb_adjusted, plot = TRUE)


### Dispersion
testDispersion(model_nb_adjusted)
check_overdispersion(model_nb_adjusted)

#### dispersion parameter, ideal < 1.5
analisis = summary(model_nb_adjusted)
analisis$deviance/analisis$df.residual

### Zero inflation - same idea of dispersion
testZeroInflation(model_nb_adjusted)
check_zeroinflation(model_nb_adjusted)
...
m_means_response <- emmeans(model_no_interaction_nb, ~ age, type="response")
cld(m_means, Letters = letters)
