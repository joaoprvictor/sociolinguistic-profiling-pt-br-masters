...
response = data$total_count_negation
response
...
summary(model_nb)
model_nb = glm(adjusted_formula, family = negative.binomial(0.925))
model_nb
summary(model_nb)
...
### R2 - not so relevant for count data but we did it anyway
r2(model_nb_adjusted)

### collinearity
check_collinearity(model_nb_adjusted)

# observing the sex levels
m_means <- emmeans(model_nb_adjusted, ~ sex)
...
#age in each level of schooling
m_means <- emmeans(model_nb_adjusted, ~ age|schooling)
cld(m_means, Letters = letters)

m_means_response <- emmeans(model_nb_adjusted, ~ age|schooling, type="response")
...
m_means <- emmeans(model_nb_adjusted, ~ schooling|age)
cld(m_means, Letters = letters)
